...
const { genrateMessage, genrateLocationMessage } = require('../src/utils/messages')
...
io.on('connection', (socket) => {
    ...
    socket.on('sendLocation', (coordinate, callback) => {
        ...
        const user = getUser(socket.id)
        io.to(user.room).emit('locationmessage', genrateLocationMessage(user.username, `https://google.com/maps?q=${coordinate.latitude},${coordinate.longitude}`))
        callback()
    })
